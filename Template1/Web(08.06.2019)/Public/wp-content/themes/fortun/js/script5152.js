...
(function($) {
  "use strict";
	...
	jQuery(document).on('ready', function(){
		...
		$(".header-navigation-menu:not(.strip-header-menu) .tab-nav-menu-content li:not('.menu-item-has-children') a").on('click', function(m){
		...
		$.fn.agni_portfolio = function(){
			...
			$portfolio_container.imagesLoaded( { background: '.portfollio-thumbnail-bg' }, function() {
				if( $portfolio_container.find($portfolio_row).data('grid') == 'fitRows' ){
					...
					//$portfolio_container.find('.grid-sizer').css({'display':'none'});
				}
